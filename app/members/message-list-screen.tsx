...
function ConvRow({ conv, onPress }: { conv: Conversation; onPress: () => void }) {
  ...
  const avatarUri = getAvatarUri(conv.partnerAvatarUrl, conv.partnerUpdatedAt);
...
function ConnectionRow({
  ...
  const avatarUri = getAvatarUri(connection.avatar_url, connection.updated_at);
...
export default function MessagesScreen() {
  ...
  const { user, apiFetch, isAuthenticated, connectionSyncTick, messageSyncTick, profileSyncTick } = useAuth();
  ...
  const loadConnections = useCallback(async () => {
    ...
    try {
      ...
      const profilesRes = await apiFetch(
        `/profiles?id=in.(${partnerIds.join(",")})&select=id,full_name,role,company,avatar_url,updated_at`
      );
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const profilesRes = await apiFetch(`/profiles?id=in.(${partnerIds.join(",")})&select=id,full_name,role,company,avatar_url,updated_at`);
      ...
      const convList: Conversation[] = profiles.map((profile: any) => {
        ...
        return {
          ...
          partnerUpdatedAt: profile.updated_at ?? null,
  ...
  useEffect(() => {
    load();
  }, [load, messageSyncTick, profileSyncTick]);
